...
const RankingSection = ({ distribuidoraId, distribuidoraNome }: Props) => {
  ...
  useEffect(() => {
    ...
    supabase
      .from("notas_empresas")
      .select(
        "empresa_id, desconto_percentual, seguranca_juridica, reputacao_reclame_aqui, valor_minimo_fatura, nota_final, nivel_risco, empresas(nome, tipo_fornecedor)"
      )
      ...
      .then(({ data }) => {
        const mapped: Company[] = (data ?? []).map((row: any, idx: number) => ({
          ...
          nivelRisco: row.nivel_risco ?? null,
        }));
        setCompanies(mapped);
        setLoading(false);
      });
